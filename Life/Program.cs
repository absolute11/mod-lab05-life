...
namespace cli_life
{
    ...
    public class Board
    {
        ...
        private void BFS(int startX, int startY, bool[,] visited, List<(int, int)> component)
        {
    ...
    public static class FigureClassifier
    {
        ...
        private static bool MatchesPattern(
            List<(int, int)> normalized,
            List<(int dx, int dy)> pattern)
        {
    ...
    {
        public static int GenerationsToStability(
            Board board,
            int maxGen       = 500,
            int stableWindow = 10)
        {
    ...
    class Program
    {
        ...
        {
            Console.Write("Число шагов (0 = бесконечно, Ctrl+C для остановки): ");
        ...
        static void RunStabilityExperiment()
        {
            ...
            {
                Console.WriteLine($"Не удалось сохранить PNG: {ex.Message}");
            }
